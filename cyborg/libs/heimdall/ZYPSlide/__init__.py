...
class ZYPSlide(SlideBase):
    ...
    def read(self, location=[0, 0], size=None, scale=1, greyscale=False):
        ...
        crop_scale = scale if scale>=1 else 1
        ...
        imgLen = lib.GetScaleImage(self.max_scale/crop_scale, int(crop_start_x/crop_scale), int(crop_start_y/crop_scale),
                                   math.floor(crop_width/crop_scale), int(crop_height/crop_scale), byref(img), self.slide)
        bits = np.ctypeslib.as_array(img, (imgLen,))
        crop_region = cv2.imdecode(bits, cv2.IMREAD_COLOR)
        crop_region = cv2.resize(crop_region, dsize=(round(size[0]/scale), round(size[1]/scale)))
